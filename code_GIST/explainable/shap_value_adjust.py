...
# shap values for two classes corresponding to 52 subjects
f_shap_values = []
...
for t in range(52):
    ...
    T_X_test, T_Y_test = X[100:], Y[100:]  # evaluation data
    ...
    shap_zero = np.zeros([np.sum(indexes == 0), 20, 1500, 1])  # right
    shap_one = np.zeros([np.sum(indexes == 1), 20, 1500, 1])  # left
    ...
    fin_explain = np.zeros([2, 20, 1500, 1])
    for i in range(2):
    ...
    for i in range(50):
        shap_values[0, :, i, :] = np.mean(shap_zero[:, 30*i:30*(i+1), :], 1)
        shap_values[1, :, i, :] = np.mean(shap_one[:, 30*i:30*(i+1), :], 1)
...
shap_values = shap_values / 52
f.append(shap_values)
# np.save('52_adjust_shapvalue_2',shap_values)
